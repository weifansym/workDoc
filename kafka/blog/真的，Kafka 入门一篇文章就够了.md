...
上面我们介绍了生产者的发送方式有三种：**不管结果如何直接发送、发送并返回结果、发送并回调**。由于消息是存在主题（topic）的分区（partition）中的，所以当 Producer 生产者发送产生
...
如果把它设置为 true，还可以通过 auto.commit.interval.ms 属性来控制提交的频率
